...
export type SerializedMessage = Message & {
  cwd: string
  userType: string
  entrypoint?: string // UR_CODE_ENTRYPOINT — distinguishes cli/sdk-ts/sdk-py/etc.
  sessionId: string
  timestamp: string
  version: string
  gitBranch?: string
 ...
 * `ur ps` can show something more useful than the last user prompt
...
/**
 * Per-file attribution state tracking UR's character contributions.
 */
export type FileAttributionState = {
  contentHash: string // SHA-256 hash of file content
  urContribution: number // Characters written by UR
  mtime: number // File modification time
}

/**
 * Attribution snapshot message stored in session transcript.
 * Tracks character-level contributions by UR for commit attribution.
 */
