...
export function MCPListPanel(t0) {
  ...
  if ($[51] !== urAiServers || $[52] !== renderServerItem) {
    t23 = urAiServers.length > 0 && <Box flexDirection="column" marginBottom={1}><Box paddingLeft={2}><Text bold={true}>claude.ai</Text></Box>{urAiServers.map(server_5 => renderServerItem(server_5))}</Box>;
    $[51] = urAiServers;
    $[52] = renderServerItem;
    $[53] = t23;
  } else {
