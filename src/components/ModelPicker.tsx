...
import { getOllamaModelOptions, mergeModelOptions } from '../utils/model/ollamaModels.js';
...
export type Props = {
   ...
   * project-scoped (written to the assistant's .ur/settings.json via
   * install.ts) and should not leak to the user's global ~/.ur/settings.
   */
...
export function ModelPicker(t0) {
  ...
  useEffect(() => {
    ...
    getOllamaModelOptions(controller.signal).then(options => {
  ...
  const baseModelOptions = t3;
  const modelOptions = useMemo(() => mergeModelOptions(baseModelOptions, ollamaModelOptions), [baseModelOptions, ollamaModelOptions]);
  let t4;
  bb0: {
  ...
  const t16 = headerText ?? "Switch between UR models. Applies to this session and future UR sessions. For other/previous model names, specify with --model.";
  let t17;
  if ($[42] !== t16) {
