 ...
 * reply and emits notifications/ur/channel/permission with
 ...
 * capabilities.experimental['ur/channel/permission'].
 *
 * Kenneth's "would this let UR self-approve?": the approving party is
 * the human via the channel, not UR. But the trust boundary isn't the
...
export type ChannelPermissionCallbacks = {
   ...
   *  (notifications/ur/channel/permission). Returns true if the ID
 ...
 * reply and emits notifications/ur/channel/permission with {request_id,
 ...
 * (notifications/ur/channel/permission) with the structured payload.
