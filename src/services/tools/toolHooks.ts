...
import { recordAction } from '../../stability/index.js'
import { filesFromArgs, markToolStart, takeToolDuration } from '../../stability/ledger.js'
...
export async function* runPostToolUseHooks<Input extends AnyObject, Output>(
  ...
  // UR stability ledger: record the successful tool use (best-effort).
  recordAction(getCwd(), {
    ts: new Date().toISOString(),
    tool: tool.name,
    args: toolInput,
    ok: true,
    durationMs: takeToolDuration(toolUseID),
    mode: toolUseContext.getAppState().toolPermissionContext.mode,
    filesTouched: filesFromArgs(toolInput),
  })
  try {
...
> {
  const postToolStartTime = Date.now()
  // UR stability ledger: record the failed tool use + error (best-effort).
  recordAction(getCwd(), {
    ts: new Date().toISOString(),
    tool: tool.name,
    args: processedInput as Record<string, unknown>,
    ok: false,
    error,
    durationMs: takeToolDuration(toolUseID),
    mode: toolUseContext.getAppState().toolPermissionContext.mode,
    filesTouched: filesFromArgs(processedInput as Record<string, unknown>),
  })
  try {
    const appState = toolUseContext.getAppState()
    const permissionMode = appState.toolPermissionContext.mode
...
> {
  ...
  // UR stability ledger: mark tool start so post/failure hooks can record latency.
  markToolStart(toolUseID)
  try {
    const appState = toolUseContext.getAppState()
