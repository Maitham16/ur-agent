 ...
 * Based on the backend API contract from urhq/urhq#250711.
 */
...
export const TeamMemoryContentSchema = lazySchema(() =>
  z.object({
    ...
    // urhq/urhq#283027. Optional for forward-compat with older
...
/**
 * Full response from GET /api/ur/team_memory
 */
...
/**
 * Structured 413 error body from the server (urhq/urhq#293258).
 ...
 * Secret rule ID is recorded — never the secret value itself.
 */
...
export type TeamMemorySyncUploadResult = {
  ...
  httpStatus?: number
  /**
   * Structured error_code from a parsed 413 body (urhq/urhq#293258).
