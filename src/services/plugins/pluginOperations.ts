 ...
 * - CLI commands (`ur plugin install/uninstall/enable/disable/update`)
...
/**
 * Is this plugin enabled (value === true) in .ur/settings.json?
 *
...
export async function uninstallPluginOp(
  ...
  if (!scopeInstallation) {
    ...
    if (actualScope !== scope && installations && installations.length > 0) {
      // Project scope is special: .ur/settings.json is shared with the team.
...
export async function setPluginEnabledOp(
  ...
  // without touching the shared .ur/settings.json).
...
async function performPluginUpdate({
  ...
  if (typeof entry.source !== 'string') {
    ...
    const manifestPath = join(sourcePath, '.ur-plugin', 'plugin.json')
    try {
