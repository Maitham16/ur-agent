 ...
 * 1. SSETransport (SSE reads + POST writes) when UR_CODE_USE_CCR_V2 is set
 * 2. HybridTransport (WS reads + POST writes) when UR_CODE_POST_FOR_SESSION_INGRESS_V2 is set
...
export function getTransportForUrl(
  ...
  if (url.protocol === 'ws:' || url.protocol === 'wss:') {
    if (isEnvTruthy(process.env.UR_CODE_POST_FOR_SESSION_INGRESS_V2)) {
