 ...
 * Handles ${UR_PLUGIN_ROOT}, ${user_config.X}, and general ${VAR} substitution
...
export function resolvePluginMcpEnvironment(
  ...
  switch (config.type) {
    ...
    case 'stdio': {
      ...
      // Resolve environment variables and add UR_PLUGIN_ROOT / UR_PLUGIN_DATA
      const resolvedEnv: Record<string, string> = {
        UR_PLUGIN_ROOT: plugin.path,
        UR_PLUGIN_DATA: getPluginDataDir(plugin.source),
        ...(stdioConfig.env || {}),
      }
      for (const [key, value] of Object.entries(resolvedEnv)) {
        if (key !== 'UR_PLUGIN_ROOT' && key !== 'UR_PLUGIN_DATA') {
    ...
    // For other types (sse-ide, ws-ide, sdk, urai-proxy), pass through unchanged
    case 'sse-ide':
    case 'ws-ide':
    case 'sdk':
    case 'urai-proxy':
