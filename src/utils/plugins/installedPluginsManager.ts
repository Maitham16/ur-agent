 ...
 *
 * The enabled/disabled state remains in .ur/settings.json for per-repo control.
 *
 ...
 *
 * Legacy cache structure: ~/.ur/plugins/cache/{plugin-name}/
 * Versioned cache structure: ~/.ur/plugins/cache/{marketplace}/{plugin}/{version}/
 *
...
function migrateV1ToV2(v1Data: InstalledPluginsFileV1): InstalledPluginsFileV2 {
  ...
  for (const [pluginId, plugin] of Object.entries(v1Data.plugins)) {
    // V2 format uses versioned cache path: ~/.ur/plugins/cache/{marketplace}/{plugin}/{version}
 ...
 *
 * getOriginalCwd() (not getCwd()) because "current project" is where UR
...
function getPluginVersionFromManifest(
  ...
  const fs = getFsImplementation()
  const manifestPath = join(pluginCachePath, '.ur-plugin', 'plugin.json')
    ...
    if (existingInstallations && existingInstallations.length > 0) {
      ...
      try {
        ...
        if (typeof entry.source === 'string') {
          ...
          // Only read manifest if the .ur-plugin dir is present
          if (dirEntries.includes('.ur-plugin')) {
