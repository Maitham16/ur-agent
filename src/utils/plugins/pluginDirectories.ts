 ...
 * - CLI flag: --cowork
 * - Environment variable: UR_CODE_USE_COWORK_PLUGINS
 *
 * The base directory can be overridden via UR_CODE_PLUGIN_CACHE_DIR.
 */
...
function getPluginsDirectoryName(): string {
  ...
  if (isEnvTruthy(process.env.UR_CODE_USE_COWORK_PLUGINS)) {
    return COWORK_PLUGINS_DIR
  }
  return PLUGINS_DIR
}
 ...
 *
 * Priority:
 * 1. UR_CODE_PLUGIN_CACHE_DIR env var (explicit override)
 * 2. Default: ~/.ur/plugins or ~/.ur/cowork_plugins
 */
export function getPluginsDirectory(): string {
  // expandTilde: when UR_CODE_PLUGIN_CACHE_DIR is set via settings.json
  // `env` (not shell), ~ is not expanded by the shell. Without this, a value
  // like "~/.ur/plugins" becomes a literal `~` directory created in the
  // cwd of every project (gh-30794 / CC-212).
  const envOverride = process.env.UR_CODE_PLUGIN_CACHE_DIR
  if (envOverride) {
    return expandTilde(envOverride)
  }
  return join(getURConfigHomeDir(), getPluginsDirectoryName())
}
 ...
 * image and point UR_CODE_PLUGIN_SEED_DIR at it. CC will use it as a
...
export function getPluginSeedDirs(): string[] {
  ...
  const raw = process.env.UR_CODE_PLUGIN_SEED_DIR
  if (!raw) return []
 ...
 * ${UR_PLUGIN_DATA}. Unlike the version-scoped install cache
 * (${UR_PLUGIN_ROOT}, which is orphaned and GC'd on every update),
 ...
 * .replace() so this isn't invoked unless ${UR_PLUGIN_DATA} is present
