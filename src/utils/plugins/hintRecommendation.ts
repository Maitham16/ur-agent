 ...
 * `<ur-hint />` tag to stderr (detected by the Bash/PowerShell tools).
 *
 * State persists in GlobalConfig.urCodeHints — a show-once record per
...
/**
 * Hard cap on `urCodeHints.plugin[]` — bounds config growth. Each shown
...
export function markHintPluginShown(pluginId: string): void {
  saveGlobalConfig(current => {
    const existing = current.urCodeHints?.plugin ?? []
...
export function disableHintRecommendations(): void {
  saveGlobalConfig(current => {
    if (current.urCodeHints?.disabled) return current
    return {
      ...current,
      urCodeHints: { ...current.urCodeHints, disabled: true },
    }
  })
}
