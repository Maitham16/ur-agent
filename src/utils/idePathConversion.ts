 ...
 * Handles conversions between UR's environment and the IDE's environment
 */
...
export interface IDEPathConverter {
  /**
   * Convert path from IDE format to UR's local format
   * Used when reading workspace folders from IDE lockfile
   */
  toLocalPath(idePath: string): string

  /**
   * Convert path from UR's local format to IDE format
...
/**
 * Converter for Windows IDE + WSL UR scenario
 */
