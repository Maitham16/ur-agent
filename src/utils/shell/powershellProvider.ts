 ...
 * shellquote.quote() which would otherwise
  ...
  return {
    ...
    async buildExecCommand(
      ...
      const cwdFilePath =
        opts.useSandbox && opts.sandboxTmpDir
          ? posixJoin(opts.sandboxTmpDir, `ur-pwd-ps-${opts.id}`)
          : join(tmpdir(), `ur-pwd-ps-${opts.id}`)
    ...
    async getEnvironmentOverrides(): Promise<Record<string, string>> {
      ...
      if (currentSandboxTmpDir) {
        ...
        env.UR_CODE_TMPDIR = currentSandboxTmpDir
      }
      return env
    },
  }
