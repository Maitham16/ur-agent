...
> {
  ...
  try {
    ...
    // Filter for UR models (SYSTEM_DEFINED filtering handled in query)
    return allProfiles
...
async function createBedrockClient() {
  ...
  // Match the UR Bedrock SDK's region behavior exactly:
  ...
  const skipAuth = isEnvTruthy(process.env.UR_CODE_SKIP_BEDROCK_AUTH)
 ...
 * For example:
 * - "eu.anthropic.ur-sonnet-4-5-20250929-v1:0" → "eu"
 * - "us.anthropic.ur-3-7-sonnet-20250219-v1:0" → "us"
 * - "arn:aws:bedrock:ap-northeast-2:123:inference-profile/global.anthropic.ur-opus-4-6-v1" → "global"
 * - "anthropic.ur-3-5-sonnet-20241022-v2:0" → undefined (foundation model)
 * - "ur-sonnet-4-5-20250929" → undefined (first-party format)
 */
 ...
 *
 * For example:
 * - applyBedrockRegionPrefix("us.anthropic.ur-sonnet-4-5-v1:0", "eu") → "eu.anthropic.ur-sonnet-4-5-v1:0"
 * - applyBedrockRegionPrefix("anthropic.ur-sonnet-4-5-v1:0", "eu") → "eu.anthropic.ur-sonnet-4-5-v1:0"
 * - applyBedrockRegionPrefix("ur-sonnet-4-5-20250929", "eu") → "ur-sonnet-4-5-20250929" (not a Bedrock model)
 */
