...
/**
 * Generates UR specific snapshot content
 * This content is always included regardless of user configuration
 */
async function getURCodeSnapshotContent(): Promise<string> {
...
RIPGREP_FUNC_END
  ...
  // consistent fast behavior in UR's shell.
...
async function getSnapshotScript(
  ...
  // Generate the user content and UR content
  const userContent = configFileExists
    ...
    : !isZsh
      ? // we need to manually force alias expansion in bash - normally `getUserSnapshotContent` takes care of this
        'echo "shopt -s expand_aliases" >> "$SNAPSHOT_FILE"'
      : ''
  const urCodeContent = await getURCodeSnapshotContent()
 ...
 *
 * This function is a critical part of UR CLI's shell integration strategy. It:
 *
 ...
 *
 * This approach allows UR CLI to execute commands as if they were run in the user's
...
export const createAndSaveSnapshot = async (
  ...
  return new Promise(async resolve => {
    try {
      ...
      if (!configFileExists) {
        logForDebugging(
          `Shell config file not found: ${configFile}, creating snapshot with UR defaults only`,
        )
      }
      ...
      const snapshotsDir = join(getURConfigHomeDir(), 'shell-snapshots')
      ...
      execFile(
        ...
        {
          env: {
            ...((process.env.UR_CODE_DONT_INHERIT_ENV
              ? {}
        ...
        async (error, stdout, stderr) => {
          if (error) {
            ...
            logForDebugging(`  - UR home: ${getURConfigHomeDir()}`)
