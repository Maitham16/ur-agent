 ...
 * Used for managing ur aliases and PATH entries
 */
...
import { getLocalURPath } from './localInstaller.js'

export const UR_ALIAS_REGEX = /^\s*alias\s+ur\s*=/
...
/**
 * Filter out installer-created ur aliases from an array of lines
 * Only removes aliases pointing to $HOME/.ur/local/ur
 ...
 */
export function filterURAliases(lines: string[]): {
  filtered: string[]
  hadAlias: boolean
} {
  let hadAlias = false
  const filtered = lines.filter(line => {
    // Check if this is a ur alias
    if (UR_ALIAS_REGEX.test(line)) {
      // Extract the alias target - handle spaces, quotes, and various formats
      // First try with quotes
      let match = line.match(/alias\s+ur\s*=\s*["']([^"']+)["']/)
      if (!match) {
        // Try without quotes (capturing until end of line or comment)
        match = line.match(/alias\s+ur\s*=\s*([^#\n]+)/)
      }
...
/**
 * Check if a ur alias exists in any shell config file
 * Returns the alias target if found, null otherwise
 * @param options Optional overrides for testing (env, homedir)
 */
export async function findURAlias(
  options?: ShellConfigOptions,
): Promise<string | null> {
  ...
  for (const configPath of Object.values(configs)) {
    ...
    for (const line of lines) {
      if (UR_ALIAS_REGEX.test(line)) {
        ...
        const match = line.match(/alias\s+ur=["']?([^"'\s]+)/)
        if (match && match[1]) {
          return match[1]
        }
      }
...
/**
 * Check if a ur alias exists and points to a valid executable
 * Returns the alias target if valid, null otherwise
 * @param options Optional overrides for testing (env, homedir)
 */
export async function findValidURAlias(
  options?: ShellConfigOptions,
): Promise<string | null> {
  const aliasTarget = await findURAlias(options)
