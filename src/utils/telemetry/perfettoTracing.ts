/**
 * Perfetto Tracing for UR (Ant-only)
 *
 ...
 *
 * Usage:
 * 1. Enable via UR_CODE_PERFETTO_TRACE=1 or UR_CODE_PERFETTO_TRACE=<path>
 * 2. Optionally set UR_CODE_PERFETTO_WRITE_INTERVAL_S=<positive integer> to write the
 *    trace file periodically (default: write only on exit).
 * 3. Run UR normally
 * 4. Trace file is written to ~/.ur/traces/trace-<session-id>.json
...
export function initializePerfettoTracing(): void {
  const envValue = process.env.UR_CODE_PERFETTO_TRACE
  logForDebugging(
  ...
  if (feature('PERFETTO_TRACING')) {
    ...
    if (isEnvTruthy(envValue)) {
      const tracesDir = join(getURConfigHomeDir(), 'traces')
    ...
    // Start periodic full-trace write if UR_CODE_PERFETTO_WRITE_INTERVAL_S is a positive integer
    const intervalSec = parseInt(
      process.env.UR_CODE_PERFETTO_WRITE_INTERVAL_S ?? '',
      10,
    )
    if (intervalSec > 0) {
