// Scheduled prompts, stored in <project>/.ur/scheduled_tasks.json.
//
...
const CRON_FILE_REL = join('.ur', 'scheduled_tasks.json')
...
/**
 * Read and parse .ur/scheduled_tasks.json. Returns an empty task list if the file
...
/**
 * Overwrite .ur/scheduled_tasks.json with the given tasks. Creates .ur/ if
...
export async function writeCronTasks(
  ...
  await mkdir(join(root, '.ur'), { recursive: true })
 ...
 * written to .ur/scheduled_tasks.json and dies with the process. The
 ...
 * and recurring tasks — a recurring task whose window passed while UR
