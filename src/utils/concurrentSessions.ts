...
function getSessionsDir(): string {
  return join(getURConfigHomeDir(), 'sessions')
}

/**
 * Kind override from env. Set by the spawner (`ur --bg`, daemon
...
function envSessionKind(): SessionKind | undefined {
  if (feature('BG_SESSIONS')) {
    const k = process.env.UR_CODE_SESSION_KIND
...
/**
 * True when this REPL is running inside a `ur --bg` tmux session.
 ...
 * typescript, python, -p), bg/daemon spawns — so `ur ps` sees everything
  ...
  try {
    ...
    await writeFile(
      ...
      jsonStringify({
        ...
        ...(feature('UDS_INBOX')
          ? { messagingSocketPath: process.env.UR_CODE_MESSAGING_SOCKET }
          : {}),
        ...(feature('BG_SESSIONS')
          ? {
              name: process.env.UR_CODE_SESSION_NAME,
              logPath: process.env.UR_CODE_SESSION_LOG,
              agent: process.env.UR_CODE_AGENT,
            }
          : {}),
      }),
    )
    // --resume / /resume mutates getSessionId() via switchSession. Without
    // this, the PID file's sessionId goes stale and `ur ps` sparkline
...
/**
 * Push live activity state for `ur ps`. Fire-and-forget from REPL's
...
export async function countConcurrentSessions(): Promise<number> {
  ...
  for (const file of files) {
    ...
    if (isProcessRunning(pid)) {
      ...
      // ~/.ur/sessions/ is shared with Windows-native UR (symlink
      // or UR_CONFIG_DIR), a Windows PID won't be probeable from WSL
