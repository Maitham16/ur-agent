      ...
      case 'macos': {
        ...
        try {
          ...
          if (stats.isDirectory()) {
            logForDebugging(
              `[UR in Chrome] Detected browser: ${config.name}`,
            )
            return browserId
          }
        } catch (e) {
...
export function isURInChromeMCPServer(name: string): boolean {
  return normalizeNameForMCP(name) === UR_IN_CHROME_MCP_SERVER_NAME
}

const MAX_TRACKED_TABS = 200
const trackedTabIds = new Set<number>()

export function trackURInChromeTabId(tabId: number): void {
...
export function isTrackedURInChromeTabId(tabId: number): boolean {
...
export async function openInChrome(url: string): Promise<boolean> {
  ...
  if (!browser) {
    logForDebugging('[UR in Chrome] No compatible browser found')
    return false
  }
...
export function getAllSocketPaths(): string[] {
  ...
  try {
    // eslint-disable-next-line custom-rules/no-sync-fs -- URForChromeContext.getSocketPaths (external @ant/ur-for-chrome-mcp) requires a sync () => string[] callback
