 ...
 * UR hints protocol.
 *
 * CLIs and SDKs running under UR can emit a self-closing
 * `<ur-hint />` tag to stderr (merged into stdout by the shell
 ...
 *
 * See docs/ur-hints.md for the vendor-facing spec.
 */

import { logForDebugging } from './debug.js'
import { createSignal } from './signal.js'

export type URCodeHintType = 'plugin'

export type URCodeHint = {
 ...
 */
const HINT_TAG_RE = /^[ \t]*<ur-hint\s+([^>]*?)\s*\/>[ \t]*$/gm
...
export function extractURCodeHints(
  ...
  const stripped = output.replace(HINT_TAG_RE, rawLine => {
    ...
    if (!SUPPORTED_VERSIONS.has(v)) {
      logForDebugging(
        `[urCodeHints] dropped hint with unsupported v=${attrs.v}`,
      )
      return ''
    }
    if (!type || !SUPPORTED_TYPES.has(type)) {
      logForDebugging(
        `[urCodeHints] dropped hint with unsupported type=${type}`,
      )
      return ''
    }
    if (!value) {
      logForDebugging('[urCodeHints] dropped hint with empty value')
      return ''
    }

    hints.push({ v, type: type as URCodeHintType, value, sourceCommand })
    return ''
  })
...
let pendingHint: URCodeHint | null = null
...
export function setPendingHint(hint: URCodeHint): void {
...
export function getPendingHintSnapshot(): URCodeHint | null {
  return pendingHint
}
...
/** Test-only reset. */
export function _resetURCodeHintStore(): void {
  pendingHint = null
