 ...
 * - Checks environment kind (UR_CODE_ENVIRONMENT_KIND)
 * - Retrieves session access token
 * - Requires UR_CODE_REMOTE_SESSION_ID for session ID
 *
...
export async function runFilePersistence(
  ...
  if (!sessionId) {
    logError(
      new Error(
        'File persistence enabled but UR_CODE_REMOTE_SESSION_ID is not set',
      ),
    )
    return null
  }
