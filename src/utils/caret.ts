 ...
 *     ↓
 * All caret operations use normalized text/offsets
 *     ↓
 * Display uses normalized text from wrappedLines
 *
 * This flow ensures consistent Unicode handling:
 * - NFD/NFC normalization differences don't break caret movement
...
export class Caret {
  ...
  constructor(
    ...
    // it's ok for the caret to be 1 char beyond the end of the string
  ...
  static fromText(
    ...
    // make MeasuredText on less than columns width, to account for caret
    return new caret(new MeasuredText(text, columns - 1), offset, selection)
  }
  ...
  render(
    ...
    return allLines
      ...
      .map((text, i) => {
        ...
        // looking for the line with the caret
        if (line !== currentLine) return displayText.trimEnd()

        // Split the line into before/at/after caret in a single pass over the
        // graphemes, accumulating display width until we reach the caret column.
        ...
        // always a grapheme boundary, so the "caret in the middle of a
        // multi-codepoint character" branch was unreachable.
        let beforecaret = ''
        let atcaret = caretChar
        let aftercaret = ''
        let currentWidth = 0
        ...
        // Only invert the caret if we have a caret character to show
        // When ghost text is present and caret is at end, show first ghost char in caret
        let renderedcaret: string
        let ghostSuffix = ''
        if (
          ghostText &&
          ...
          // First ghost character goes in the inverted caret (grapheme-safe)
          const firstGhostChar =
            firstGrapheme(ghostText.text) || ghostText.text[0]!
          renderedcaret = caretChar ? invert(firstGhostChar) : firstGhostChar
          // Rest of ghost text is dimmed after caret
          const ghostRest = ghostText.text.slice(firstGhostChar.length)
          if (ghostRest.length > 0) {
            ghostSuffix = ghostText.dim(ghostRest)
          }
        } else {
          renderedcaret = caretChar ? invert(atcaret) : atcaret
        }

        return (
          beforecaret + renderedcaret + ghostSuffix + aftercaret.trimEnd()
        )
      })
      .join('\n')
  }
  ...
  left(): caret {
    ...
    if (chip) return new caret(this.measuredText, chip.start)

    const prevOffset = this.measuredText.prevOffset(this.offset)
    return new caret(this.measuredText, prevOffset)
  }
  ...
  right(): caret {
    ...
    if (chip) return new caret(this.measuredText, chip.end)

    const nextOffset = this.measuredText.nextOffset(this.offset)
    return new caret(this.measuredText, Math.min(nextOffset, this.text.length))
  }

  /**
   * If an [Image #N] chip ends at `offset`, return its bounds. Used by left()
   * to hop the caret over the chip instead of stepping into it.
   */
  ...
  up(): caret {
    ...
    if (column > prevLineDisplayWidth) {
      ...
      return new caret(this.measuredText, newOffset, 0)
    }
   ...
   */
  private startOfCurrentLine(): caret {
    const { line } = this.getPosition()
    return new caret(
      this.measuredText,
      this.getOffset({
  ...
  firstNonBlankInLine(): caret {
    ...
    return new caret(this.measuredText, offset, 0)
  }
  ...
  // Helper to create caret with preserved column, clamped to line length
  // Snaps to grapheme boundary to avoid landing mid-grapheme
  private createcaretWithColumn(
    lineStart: number,
    lineEnd: number,
    targetColumn: number,
  ): caret {
  ...
  endOfLogicalLine(): caret {
    return new caret(this.measuredText, this.findLogicalLineEnd(), 0)
  }

  startOfLogicalLine(): caret {
    return new caret(this.measuredText, this.findLogicalLineStart(), 0)
  }

  firstNonBlankInLogicalLine(): caret {
  ...
  upLogicalLine(): caret {
    ...
    if (currentStart === 0) {
      return new caret(this.measuredText, 0, 0)
    }
    ...
    return this.createcaretWithColumn(
      prevLineStart,
      prevLineEnd,
      currentColumn,
    )
  ...
  nextWord(): caret {
    ...
    for (const boundary of wordBoundaries) {
      if (boundary.isWordLike && boundary.start > this.offset) {
        return new caret(this.measuredText, boundary.start)
      }
    }

    // If no next word found, go to end
    return new caret(this.measuredText, this.text.length)
  }
  ...
  endOfWord(): caret {
    ...
    for (const boundary of wordBoundaries) {
      ...
      if (this.offset >= boundary.start && this.offset < boundary.end - 1) {
        ...
        return new caret(this.measuredText, boundary.end - 1)
      }
      ...
      if (this.offset === boundary.end - 1) {
        ...
        for (const nextBoundary of wordBoundaries) {
          if (nextBoundary.isWordLike && nextBoundary.start > this.offset) {
            return new caret(this.measuredText, nextBoundary.end - 1)
          }
        }
        return this
      }
  ...
  prevWord(): caret {
    ...
    if (prevWordStart !== null) {
      return new caret(this.measuredText, prevWordStart)
    }

    return new caret(this.measuredText, 0)
  }
  ...
  nextVimWord(): caret {
    ...
    return new caret(this.measuredText, pos)
  }
  ...
  endOfVimWord(): caret {
    ...
    if (pos >= text.length) {
      return new caret(this.measuredText, text.length)
    }
  ...
  nextWORD(): Caret {
    ...
    let nextcaret: Caret = this
    // If we're on a non-whitespace character, move to the next whitespace
    while (!nextcaret.isOverWhitespace() && !nextcaret.isAtEnd()) {
      nextcaret = nextcaret.right()
    }
  ...
  endOfWORD(): Caret {
    ...
    const atEndOfWORD =
      !caret.isOverWhitespace() &&
      (caret.right().isOverWhitespace() || caret.right().isAtEnd())
    ...
    if (caret.isOverWhitespace()) {
      caret = caret.nextWORD()
    }

    // Now move to the end of the current WORD
    while (!caret.right().isOverWhitespace() && !caret.isAtEnd()) {
      caret = caret.right()
    }
  ...
  prevWORD(): Caret {
    ...
    if (caret.left().isOverWhitespace()) {
      caret = caret.left()
    }

    // Move left over any whitespace characters
    while (caret.isOverWhitespace() && !caret.isAtStart()) {
      caret = caret.left()
    }

    // If we're over a non-whitespace character, move to the start of this WORD
    if (!caret.isOverWhitespace()) {
      while (!caret.left().isOverWhitespace() && !caret.isAtStart()) {
        caret = caret.left()
      }
    }
  ...
  modifyText(end: caret, insertString: string = ''): caret {
  ...
  insert(insertString: string): caret {
    const newcaret = this.modifyText(this, insertString)
    return newcaret
  }
  ...
  deleteToLineStart(): { caret: Caret; killed: string } {
    // If caret is right after a newline (at start of line), delete just that
    ...
    if (this.offset > 0 && this.text[this.offset - 1] === '\n') {
      return { caret: this.left().modifyText(this), killed: '\n' }
    }

    // Use startOfLine() so that at column 0 of a wrapped visual line,
    // the caret moves to the previous visual line's start instead of
    // getting stuck.
    const startcaret = this.startOfLine()
    const killed = this.text.slice(startcaret.offset, this.offset)
    return { caret: startcaret.modifyText(this), killed }
  }

  deleteToLineEnd(): { caret: Caret; killed: string } {
    // If caret is on a newline character, delete just that character
    if (this.text[this.offset] === '\n') {
      return { caret: this.modifyText(this.right()), killed: '\n' }
    }

    const endcaret = this.endOfLine()
    const killed = this.text.slice(this.offset, endcaret.offset)
    return { caret: this.modifyText(endcaret), killed }
  }

  deleteToLogicalLineEnd(): Caret {
    // If caret is on a newline character, delete just that character
    if (this.text[this.offset] === '\n') {
      return this.modifyText(this.right())
    }
  ...
  deleteWordBefore(): { caret: Caret; killed: string } {
    if (this.isAtStart()) {
      return { caret: this, killed: '' }
    }
    const target = this.snapOutOfImageRef(this.prevWord().offset, 'start')
    const prevWordcaret = new Caret(this.measuredText, target)
    const killed = this.text.slice(prevWordcaret.offset, this.offset)
    return { caret: prevWordcaret.modifyText(this), killed }
  }

  /**
   * Deletes a token before the caret if one exists.
   ...
   *
   * Returns null if no token found at caret position.
   * Only triggers when caret is at end of token (followed by whitespace or EOL).
   */
  deleteTokenBefore(): caret | null {
    // caret at chip.start is the "selected" state — backspace deletes the
    ...
    if (chipAfter) {
      ...
      return this.modifyText(new caret(this.measuredText, end))
    }
    ...
    // Only trigger if caret is at a word boundary (whitespace or end of string after caret)
    ...
    if (pasteMatch) {
      ...
      return new caret(this.measuredText, matchStart).modifyText(this)
    }

    return null
  }

  deleteWordAfter(): caret {
    if (this.isAtEnd()) {
      return this
    }

    const target = this.snapOutOfImageRef(this.nextWord().offset, 'end')
    return this.modifyText(new caret(this.measuredText, target))
  }
  ...
  startOfLastLine(): caret {
    ...
    return new caret(this.measuredText, lastNewlineIndex + 1, 0)
  }

  goToLine(lineNumber: number): caret {
      ...
      if (
        ...
        if (currentLine.isPrecededByNewline) {
          ...
          if (stringPosInLine < leadingWhitespace) {
            // caret is in the trimmed whitespace area, position at start
            displayColumn = 0
          } else {
