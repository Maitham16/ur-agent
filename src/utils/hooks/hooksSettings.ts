...
export function getAllHooks(appState: AppState): IndividualHookConfig[] {
  ...
  if (!restrictedToManagedOnly) {
    ...
    // both resolve to ~/.ur/settings.json)
...
export function hookSourceDescriptionDisplayString(source: HookSource): string {
  switch (source) {
    case 'userSettings':
      return 'User settings (~/.ur/settings.json)'
    case 'projectSettings':
      return 'Project settings (.ur/settings.json)'
    case 'localSettings':
      return 'Local settings (.ur/settings.local.json)'
    case 'pluginHook':
      ...
      // e.g., "Plugin hooks (~/.ur/plugins/repos/source/example-plugin/example-plugin/hooks/hooks.json)"
      return 'Plugin hooks (~/.ur/plugins/*/hooks/hooks.json)'
    case 'sessionHook':
      return 'Session hooks (in-memory, temporary)'
    case 'builtinHook':
      return 'Built-in hooks (registered internally by UR)'
    default:
