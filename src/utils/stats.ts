...
/**
 * Convert a PersistedStatsCache to URCodeStats by computing derived fields.
 */
...
/**
 * Aggregates stats from all UR sessions across all projects.
 * Uses a disk cache to avoid reprocessing historical data.
 */
export async function aggregateURCodeStats(): Promise<URCodeStats> {
 ...
 */
export async function aggregateURCodeStatsForRange(
...
/**
 * Convert ProcessedStats to URCodeStats.
 * Used for filtered date ranges that bypass the cache.
 */
function processedStatsToURCodeStats(
...
function getEmptyStats(): URCodeStats {
  return {
    totalSessions: 0,
    totalMessages: 0,
    totalDays: 0,
