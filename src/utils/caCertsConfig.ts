 ...
 * user-controlled files (~/.ur/settings.json and ~/.ur.json),
 ...
 *
 * We read from global config (~/.ur.json) and user settings
 * (~/.ur/settings.json). These are user-controlled files that don't
...
function getExtraCertsPathFromConfig(): string | undefined {
  try {
    ...
    // Only read from user-controlled settings (~/.ur/settings.json),
