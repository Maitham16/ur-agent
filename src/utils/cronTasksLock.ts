// Scheduler lease lock for .ur/scheduled_tasks.json.
//
// When multiple UR sessions run in the same project directory, only one
...
const LOCK_FILE_REL = join('.ur', 'scheduled_tasks.lock')
...
async function tryCreateExclusive(
  ...
  try {
    ...
    if (code === 'ENOENT') {
      // .ur/ doesn't exist yet — create it and retry once. In steady
