 ...
 * pipeline from user input to first token arrival. Enable by setting UR_CODE_PROFILE_QUERY=1
 *
 ...
 * - query_client_creation_start/end: Creating UR client
...
const ENABLED = isEnvTruthy(process.env.UR_CODE_PROFILE_QUERY)
...
function getQueryProfileReport(): string {
  if (!ENABLED) {
    return 'Query profiling not enabled (set UR_CODE_PROFILE_QUERY=1)'
  }
