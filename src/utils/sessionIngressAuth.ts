...
function getTokenFromFileDescriptor(): string | null {
  ...
  const fdEnv = process.env.UR_CODE_WEBSOCKET_AUTH_FILE_DESCRIPTOR
  if (!fdEnv) {
    ...
    const path =
      process.env.UR_SESSION_INGRESS_TOKEN_FILE ??
      CCR_SESSION_INGRESS_TOKEN_PATH
  ...
  if (Number.isNaN(fd)) {
    logForDebugging(
      `UR_CODE_WEBSOCKET_AUTH_FILE_DESCRIPTOR must be a valid file descriptor number, got: ${fdEnv}`,
      { level: 'error' },
    )
 ...
 *
 * Priority order:
 *  1. Environment variable (UR_CODE_SESSION_ACCESS_TOKEN) — set at spawn time,
 ...
 *  2. File descriptor (legacy path) — UR_CODE_WEBSOCKET_AUTH_FILE_DESCRIPTOR,
 *     read once and cached.
 *  3. Well-known file — UR_SESSION_INGRESS_TOKEN_FILE env var path, or
 *     /home/ur/.ur/remote/.session_ingress_token. Covers subprocesses
 *     that can't inherit the FD.
 */
export function getSessionIngressAuthToken(): string | null {
  // 1. Check environment variable
  const envToken = process.env.UR_CODE_SESSION_ACCESS_TOKEN
  if (envToken) {
    return envToken
  }
...
export function getSessionIngressAuthHeaders(): Record<string, string> {
  ...
  if (token.startsWith('sk-ant-sid')) {
    ...
    const orgUuid = process.env.UR_CODE_ORGANIZATION_UUID
    if (orgUuid) {
...
export function updateSessionIngressAuthToken(token: string): void {
  process.env.UR_CODE_SESSION_ACCESS_TOKEN = token
}
