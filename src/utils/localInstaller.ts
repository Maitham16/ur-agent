...
// hfi.tsx get a chance to set UR_CONFIG_DIR in main(), and would also
// populate the memoize cache with that stale value for all 150+ other callers.
function getLocalInstallDir(): string {
  return join(getURConfigHomeDir(), 'local')
}
export function getLocalURPath(): string {
  return join(getLocalInstallDir(), 'ur')
}
...
export function isRunningFromLocalInstallation(): boolean {
  ...
  return execPath.includes('/.ur/local/node_modules/')
}
...
export async function ensureLocalPackageEnvironment(): Promise<boolean> {
  try {
    ...
    await writeIfMissing(
      ...
      jsonStringify(
        { name: 'ur-local', version: '0.0.1', private: true },
        null,
        2,
      ),
    )
    ...
    const wrapperPath = join(localInstallDir, 'ur')
    const created = await writeIfMissing(
      wrapperPath,
      `#!/bin/sh\nexec "${localInstallDir}/node_modules/.bin/ur" "$@"`,
      0o755,
    )
    if (created) {
...
/**
 * Install or update UR CLI package in the local directory
 ...
 */
export async function installOrUpdateURPackage(
  ...
  try {
    ...
    if (result.code !== 0) {
      const error = new Error(
        `Failed to install UR CLI package: ${result.stderr}`,
      )
      logError(error)
...
export async function localInstallationExists(): Promise<boolean> {
  try {
    await access(join(getLocalInstallDir(), 'node_modules', '.bin', 'ur'))
    return true
  } catch {
    return false
  }
