...
export function getBinaryName(platform: string): string {
  return platform.startsWith('win32') ? 'ur.exe' : 'ur'
}
...
function getBaseDirectories() {
  ...
  return {
    ...
    versions: join(getXDGDataHome(), 'ur', 'versions'),

    // Cache directories (can be deleted)
    staging: join(getXDGCacheHome(), 'ur', 'staging'),

    // State directories
    locks: join(getXDGStateHome(), 'ur', 'locks'),
...
async function isPossibleURBinary(filePath: string): Promise<boolean> {
  try {
...
async function installVersionFromPackage(
  ...
  try {
    ...
    const nodeModulesDir = join(stagingPath, 'node_modules', '@urhq-ai')
    const entries = await readdir(nodeModulesDir)
    const nativePackage = entries.find((entry: string) =>
      entry.startsWith('ur-cli-native-'),
    )
...
async function performVersionUpdate(
  ...
  // Create direct symlink from ~/.local/bin/ur to the version binary
  ...
  if (!(await isPossibleURBinary(executablePath))) {
...
async function versionIsAvailable(version: string): Promise<boolean> {
  ...
  return isPossibleURBinary(installPath)
}
...
async function updateLatest(
  ...
  if (
    ...
    (await isPossibleURBinary(executablePath))
  ) {
...
export async function checkInstall(
  ...
  // Check if ur executable exists and is valid.
  ...
  // isPossibleURBinary stats the path internally, so we don't pre-check
  // with access() — that would be a TOCTOU between access and the stat.
  if (isWindows) {
    // On Windows it's a copied executable, not a symlink
    if (!(await isPossibleURBinary(dirs.executable))) {
      messages.push({
        message: `installMethod is native, but ur command is missing or invalid at ${dirs.executable}`,
        userActionRequired: true,
        type: 'error',
      })
    }
    ...
    try {
      ...
      if (isENOENT(e)) {
        messages.push({
          message: `installMethod is native, but ur command not found at ${dirs.executable}`,
          userActionRequired: true,
          type: 'error',
        })
      } else {
        ...
        if (!(await isPossibleURBinary(dirs.executable))) {
          messages.push({
            message: `${dirs.executable} exists but is not a valid UR binary`,
            userActionRequired: true,
            type: 'error',
          })
        }
  ...
  if (getPlatform().startsWith('win32')) {
    ...
    try {
      ...
      for (const file of files) {
        if (!/^ur\.exe\.old\.\d+$/.test(file)) continue
        try {
...
/**
 * Remove the ur symlink from the executable directory
...
export async function removeInstalledSymlink(): Promise<void> {
  ...
  try {
    ...
    logForDebugging(`Removed ur symlink at ${dirs.executable}`)
  } catch (error) {
    if (isENOENT(error)) {
      return
    }
    logError(new Error(`Failed to remove ur symlink: ${error}`))
  }
}

/**
 * Clean up old ur aliases from shell configuration files
...
export async function cleanupShellAliases(): Promise<SetupMessage[]> {
  ...
  for (const [shellType, configFile] of Object.entries(configMap)) {
    try {
      ...
      const { filtered, hadAlias } = filterURAliases(lines)

      if (hadAlias) {
        await writeFileLines(configFile, filtered)
        messages.push({
          message: `Removed ur alias from ${configFile}. Run: unalias ur`,
          userActionRequired: true,
          type: 'alias',
        })
        logForDebugging(`Cleaned up ur alias from ${shellType} config`)
      }
    } catch (error) {
      logError(error)
      messages.push({
...
async function manualRemoveNpmPackage(
  ...
  try {
    ...
    if (getPlatform().startsWith('win32')) {
      ...
      const binCmd = join(globalPrefix, 'ur.cmd')
      const binPs1 = join(globalPrefix, 'ur.ps1')
      const binExe = join(globalPrefix, 'ur')
      ...
      const binSymlink = join(globalPrefix, 'bin', 'ur')
...
export async function cleanupNpmInstallations(): Promise<{
  ...
  // Always attempt to remove @urhq-ai/ur
  ...
  if (MACRO.PACKAGE_URL && MACRO.PACKAGE_URL !== '@urhq-ai/ur') {
  ...
  // Check for local installation at ~/.ur/local
  const localInstallDir = join(homedir(), '.ur', 'local')
