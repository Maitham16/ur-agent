 ...
 *
 * This module provides PID-based locking for running UR versions.
...
/**
 * Validate that a running process is actually a UR process
 * This helps mitigate PID reuse issues
 */
function isURProcess(pid: number, expectedExecPath: string): boolean {
  if (!isProcessRunning(pid)) {
    return false
  }

  // If the PID matches our current process, we know it's valid
  // This handles test environments where the command might not contain 'ur'
  if (pid === process.pid) {
    return true
  }
  ...
  try {
    ...
    // Check if the command contains 'ur' or the expected exec path
...
export function isLockActive(lockFilePath: string): boolean {
  ...
  // Secondary validation: is it actually a UR process?
  // This helps with PID reuse scenarios
  if (!isURProcess(pid, execPath)) {
    logForDebugging(
      `Lock PID ${pid} is running but does not appear to be UR - treating as stale`,
    )
    return false
  }
...
export async function tryAcquireLock(
  ...
  // validates it's actually a UR process (to handle PID reuse scenarios)
