 ...
 * Terminal Launcher
 *
 * Detects the user's preferred terminal emulator and launches UR
...
/**
 * Launch UR in the detected terminal emulator.
 *
 ...
 *
 * For pure-argv paths: urPath, --prefill, query, cwd travel as distinct
 * argv elements end-to-end. No sh -c. No shellQuote(). The terminal does
 * chdir(cwd) and execvp(ur, argv). Spaces/quotes/metacharacters in
...
export async function launchInTerminal(
  ...
  const urArgs = ['--deep-link-origin']
  if (action.repo) {
    urArgs.push('--deep-link-repo', action.repo)
    if (action.lastFetchMs !== undefined) {
      urArgs.push('--deep-link-last-fetch', String(action.lastFetchMs))
    }
  }
  if (action.query) {
    urArgs.push('--prefill', action.query)
  }

  switch (process.platform) {
    case 'darwin':
      return launchMacosTerminal(terminal, urPath, urArgs, action.cwd)
    case 'linux':
      return launchLinuxTerminal(terminal, urPath, urArgs, action.cwd)
    case 'win32':
      return launchWindowsTerminal(terminal, urPath, urArgs, action.cwd)
    default:
      return false
  }
}
...
async function launchMacosTerminal(
  ...
  switch (terminal.command) {
    ...
    case 'iTerm': {
      const shCmd = buildShellCommand(urPath, urArgs, cwd)
...
async function launchWindowsTerminal(
  ...
  switch (terminal.name) {
    ...
    // mode that also keeps the session interactive after ur exits.
    ...
    case 'PowerShell': {
      ...
      args.push(
        '-NoExit',
        '-Command',
        `${cdCmd}& ${psQuote(urPath)} ${urArgs.map(psQuote).join(' ')}`,
      )
      break
    }
    ...
    default: {
      ...
      args.push(
        '/k',
        `${cdCmd}${cmdQuote(urPath)} ${urArgs.map(a => cmdQuote(a)).join(' ')}`,
      )
      break
    }
  }
...
function buildShellCommand(
  ...
  return `${cdPrefix}${[urPath, ...urArgs].map(shellQuote).join(' ')}`
}
 ...
 * *child process* (ur.exe) uses CommandLineToArgvW, where a trailing
