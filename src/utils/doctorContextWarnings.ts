...
async function checkAgentMdFiles(): Promise<ContextWarning | null> {
  ...
  const message =
    largeFiles.length === 1
      ? `Large UR.md file detected (${largeFiles[0]!.content.length.toLocaleString()} chars > ${MAX_MEMORY_CHARACTER_COUNT.toLocaleString()})`
      : `${largeFiles.length} large UR.md files detected (each > ${MAX_MEMORY_CHARACTER_COUNT.toLocaleString()} chars)`
...
export async function checkContextWarnings(
  ...
  const [urMdWarning, agentWarning, mcpWarning, unreachableRulesWarning] =
    await Promise.all([
      checkAgentMdFiles(),
