...
export async function getURDesktopConfigPath(): Promise<string> {
  const platform = getPlatform()

  if (!SUPPORTED_PLATFORMS.includes(platform)) {
    throw new Error(
      `Unsupported platform: ${platform} - UR Desktop integration only works on macOS and WSL.`,
    )
  }
  ...
  if (windowsHome) {
    ...
    const configPath = `/mnt/c${wslPath}/AppData/Roaming/UR/ur_desktop_config.json`
  ...
  try {
    ...
    try {
      ...
      // Look for UR Desktop config in each user directory
  ...
  throw new Error(
    'Could not find UR Desktop config file in Windows. Make sure UR Desktop is installed on Windows.',
  )
}

export async function readURDesktopMcpServers(): Promise<
  Record<string, McpServerConfig>
> {
  if (!SUPPORTED_PLATFORMS.includes(getPlatform())) {
    throw new Error(
      'Unsupported platform - UR Desktop integration only works on macOS and WSL.',
    )
  }
  try {
    const configPath = await getURDesktopConfigPath()
