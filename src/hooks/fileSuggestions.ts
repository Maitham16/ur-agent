...
/**
 * Gets additional files from UR config directories
 */
async function getURConfigFiles(cwd: string): Promise<string[]> {
  const markdownFileArrays = await Promise.all(
    UR_CONFIG_DIRECTORIES.map(subdir =>
