...
import { hasURAiMcpEverConnected } from '../../services/mcp/claudeai.js';
    ...
    t2 = () => {
      ...
      if (failedURAiClients.length > 0) {
        addNotification({
          ...
          jsx: <><Text color="error">{failedURAiClients.length} claude.ai{" "}{failedURAiClients.length === 1 ? "connector" : "connectors"}{" "}unavailable</Text><Text dimColor={true}> · /mcp</Text></>,
          priority: "medium"
        });
      }
      ...
      if (needsAuthURAiServers.length > 0) {
        addNotification({
          ...
          jsx: <><Text color="warning">{needsAuthURAiServers.length} claude.ai{" "}{needsAuthURAiServers.length === 1 ? "connector needs" : "connectors need"}{" "}auth</Text><Text dimColor={true}> · /mcp</Text></>,
          priority: "medium"
        });
      }
    };
...
function _temp4(client_2) {
  return client_2.type === "needs-auth" && client_2.config.type === "claudeai-proxy" && hasURAiMcpEverConnected(client_2.name);
}
function _temp3(client_1) {
  return client_1.type === "needs-auth" && client_1.config.type !== "claudeai-proxy";
}
function _temp2(client_0) {
  return client_0.type === "failed" && client_0.config.type === "claudeai-proxy" && hasURAiMcpEverConnected(client_0.name);
}
