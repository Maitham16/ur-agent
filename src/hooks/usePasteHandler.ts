...
export function usePasteHandler({
  ...
  const wrappedOnInput = (input: string, key: Key, event: InputEvent): void => {
    ...
    // Check if we should handle as paste. Bracketed paste is explicit via
    // isPasted. Do not infer paste solely from input length: some terminals
    // deliver ordinary typed text and Enter in one stdin read, and swallowing
    // the following Return makes Enter appear broken.
    const shouldHandleAsPaste =
      onPaste &&
      (pastePendingRef.current || hasImageFilePath || isFromPaste)
