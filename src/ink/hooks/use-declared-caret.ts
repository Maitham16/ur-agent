...
import caretDeclarationContext from '../components/caretDeclarationContext.js'
import type { DOMElement } from '../dom.js'

/**
 * Declares where the terminal caret should be parked after each frame.
 *
 * Terminal emulators render IME preedit text at the physical caret
 * position, and screen readers / screen magnifiers track the native
 * caret — so parking it at the text input's caret makes CJK input
 ...
 */
export function useDeclaredCaret({
  line,
  column,
  active,
}: {
  ...
  const setcaretDeclaration = useContext(caretDeclarationContext)
  ...
  useLayoutEffect(() => {
    ...
    if (active && node) {
      setcaretDeclaration({ relativeX: column, relativeY: line, node })
    } else {
      setcaretDeclaration(null, node)
    }
  })
  ...
  useLayoutEffect(() => {
    return () => {
      setcaretDeclaration(null, nodeRef.current)
    }
  }, [setcaretDeclaration])
